  ...
  const inner = (
    <div className="bg-white rounded-2xl border border-[#E8EEF6] p-4"
      ...
      <div className="flex items-baseline gap-1.5">
        <span className="text-[26px] font-extrabold leading-none tabular" style={{ color }}>{value}</span>
...
function QuickAction({ href, label, color, children }: {
  ...
  return (
    <Link href={href} className="flex flex-col items-center gap-2 flex-shrink-0 active:opacity-70 transition-opacity">
      ...
      <span className="text-[10px] font-semibold text-[#64748B] text-center leading-tight w-[64px]">{label}</span>
    </Link>
  )
}
...
export default async function DashboardPage() {
  ...
  const [{ data: summary }, { data: metrics }, { data: insight }, { data: activities }] = await Promise.all([
    ...
    <div className="flex flex-col min-h-dvh bg-[#F2F5FA]">
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-[#E8EEF6] px-4 py-3"
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#1D4ED8] to-[#7C3AED] flex items-center justify-center text-sm font-bold text-white">י</div>
        </div>
      </div>

      {/* Quick Actions — Maccabi blue circles */}
      <div className="bg-white border-b border-[#E8EEF6] px-4 py-4"
      ...
      <div className="flex-1 pb-4">
        ...
        <div className="px-4">
          <div className="bg-white rounded-2xl border border-[#E8EEF6] p-4"
            style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)', borderTop: '3px solid #1D4ED8' }}>
        ...
        <div className="px-4 grid grid-cols-2 gap-3">
          <MetricCard label="מאזן קלורי" unit="קל׳"
            ...
            href="/nutrition" />
          <MetricCard label="צעדים"
            value={steps ? steps.toLocaleString('he-IL') : '--'}
            sub="יעד: 10,000" color="#1D4ED8" />
          {ds && (
          ...
          <>
            ...
            <div className="px-4">
              <div className="rounded-2xl p-4 border border-[#E8EEF6]"
                style={{ background: 'linear-gradient(135deg, #FDF4FF 0%, #EFF6FF 100%)', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[9px] font-bold bg-[#DB2777] text-white px-2 py-0.5 rounded-full uppercase">✦ AI Coach</span>
                </div>
