...
export default async function DashboardPage() {
  ...
  const [
    { data: summary }, { data: metricsRaw }, { data: insightRaw },
    { data: workoutsRaw }, { data: weekRaw }, { data: profile }
  ] = await Promise.all([
  ...
  const statsData = {
    ...
    stepsPct,
    hrPct:       hr ? Math.max(0, 100 - Math.abs(hr - 65) * 4) : 0,
    calPct,
    proteinPct:  ds ? Math.min(ds.protein_g / 142 * 100, 100)  : 0,
    workoutsPct,
  }
  ...
  // שם פרטי: metadata Google -> users table -> email prefix
  const avatarUrl = (user.user_metadata?.avatar_url || user.user_metadata?.picture) as string | undefined
  const rawName   = (user.user_metadata?.full_name || user.user_metadata?.name || profile?.name || user.email?.split('@')[0] || 'משתמש') as string
  const firstName = rawName.split(/[\s._-]/)[0]

  return (
    <div className="flex min-h-dvh flex-col bg-background">
      <AppHeader
        title={`לוח הבריאות של ${firstName}`}
      ...
      <main className="flex flex-col gap-4 px-4 py-4 pb-6 flex-1">
        ...
        {ins && (
          <div className="rounded-3xl bg-card p-4 shadow-sm" style={{ borderRight: '4px solid var(--pink)' }}>
            <span
              className="inline-block mb-2 text-[9px] font-bold text-white px-2 py-0.5 rounded-full uppercase"
